 ...
 *
 * 使用方法:
 * node classify-images-v3.js --base-dir /path/to/airline --airline "国泰航空"
 */
...
const CATEGORIES = [
  ...
  { id: '1-座椅布局', description: '座位图、舱位布局平面图（俯瞰视角）' },
...
// 基于图片序号的语义分类规则（改进版）
// 根据 seatmaps.com 的图片编号规律：
// 01 = logo, 02-08 = 图标，03 = 主座位图，09-12 = 座椅细节，13 = 另一视角座位图
// 14-17 = 可能是座椅细节或额外座位图（需要判断），18 = 座椅大图，19-22 = 娱乐设备
function classifyImage(filename, size, ext) {
  ...
  // 03-image.webp = 主座椅布局图（俯瞰视角的座位分布图）
  if (index === 3 && ext === 'webp') {
  ...
  // 19-22 = 娱乐设备（IFE 屏幕、USB 端口等）
  if (index >= 19 && index <= 22 && ext === 'webp') {
    return { category: '4-娱乐设备', reason: '机上娱乐设备' };
  }
  ...
  if (ext === 'webp') {
    if (size > 50000) {
      return { category: '1-座椅布局', reason: '大尺寸图片（可能是布局图）' };
    }
...
function processAircraft(aircraftType, baseDir) {
  ...
  // 保存详细报告
  const reportPath = path.join(baseDir, aircraftType, 'images', 'classification-report-v3.md');
  let report = `# ${aircraftType} 图片分类报告 (v3)\n\n`;
...
function main() {
  ...
  // 生成汇总报告
  const summaryPath = path.join(targetDir, '图片分类汇总 -v3.md');
  let summary = `# 图片分类汇总 (v3)\n\n`;
