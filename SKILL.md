...
version: 1.0.0
description: |
  抓取并整理航空公司座位图数据、机型配置、舱位信息、用户评分与配图。
  用于 seatmaps.com、航司座位图、机型详情、图片分类、批量建立航司数据库等任务。
...
系统化收集航空公司机型资料，并整理成可复用的 `FlightData/`（或用户指定）数据目录。

## 技能包结构（标准封装）

| 文件 / 目录 | 说明 |
|-------------|------|
| `SKILL.md` | 主入口：门禁、进度规则、阶段总览、触发场景 |
| `reference.md` | 路径与依赖、输出目录规范、分阶段命令、完成检查、任务复盘 |
| `examples.md` | 典型用法与命令示例 |
| `scripts/` | 可执行脚本；清单见 `scripts/README.md` |

需要执行具体命令或核对目录树时，再读取 `reference.md`，避免重复占用上下文。

## 路径约定

- **技能根目录**：本文件所在目录（含 `scripts/`）。
- Python 依赖、`--output` 路径说明与各 Phase 命令全文：见 [reference.md](reference.md)。
...
- **输出目录结构**、**Phase 1～9 命令**、**完成检查**、**自我学习 / 复盘**：见 [reference.md](reference.md)
- **可复制示例**：见 [examples.md](examples.md)
